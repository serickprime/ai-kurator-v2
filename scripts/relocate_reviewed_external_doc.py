"""Preview or execute one reviewed external-doc canonical relocation."""
...
from app.docs_registry.canonical_relocation import (  # noqa: E402
    ReviewedCanonicalRelocationError,
    build_reviewed_canonical_relocation_plan,
    execute_reviewed_canonical_relocation,
    format_canonical_relocation_plan_text,
    load_canonical_relocation_review,
)
...
from app.docs_registry.reviewed_key_reprocessing import NoTermStatisticsRefreshRepository  # noqa: E402
...
class _SingleDocumentIdAction(argparse.Action):
    """Reject repeated --document-id instead of silently taking the last value."""
    ...
    def __call__(
        ...
        if getattr(namespace, self.dest, None) is not None:
            parser.error("--document-id accepts exactly one value")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(
        description="Preview or execute one reviewed external-doc canonical relocation.",
    )
    ...
    parser.add_argument("--review", type=Path, required=True, help="Reviewed canonical relocation artifact JSON.")
    parser.add_argument("--backup", type=Path, help="Fresh rollback-capable source baseline manifest.")
    parser.add_argument(
        "--document-id",
        required=True,
        action=_SingleDocumentIdAction,
        help="Exact old active document UUID.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Execute one reviewed canonical relocation after all gates pass.",
    )
...
async def main_async() -> int:
    """Run the relocation preview or explicit execution."""
    args = parse_args()
    ...
    try:
        ...
        result = None
        if args.confirm_relocate_reviewed:
            ...
            result = await execute_reviewed_canonical_relocation(
                plan=plan,
                fetcher=crawler,
        ...
        if args.format == "json":
            ...
            print(format_canonical_relocation_plan_text(plan))
            if result is not None:
                print("")
                print("Canonical relocation execution result")
                print(f"- status: {result.status}")
                print(f"- new document: {result.new_document_id or 'none'}")
                print(f"- old archive rows updated: {result.old_archive_rows_updated}")
                print(f"- term_statistics: {result.term_statistics_status}")
                print(f"- partial failure: {'yes' if result.partial_failure else 'no'}")
        if result is not None:
            return 0 if result.status == "relocated" else 2
        return 0 if plan.readiness else 2
    except (DocsReprocessingPlanError, ReviewedCanonicalRelocationError) as exc:
...
async def _build_live_plan(args: argparse.Namespace):
    ...
    relocation_artifact = load_canonical_relocation_review(args.review)
    ...
    plan = build_reviewed_canonical_relocation_plan(
        scope=scope,
        inventory=inventory,
        current_plan=current_plan,
        relocation_artifact=relocation_artifact,
